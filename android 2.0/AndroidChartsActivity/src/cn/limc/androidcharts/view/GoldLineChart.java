		...
		case MotionEvent.ACTION_DOWN:
			...
			if (mGoldLineEntity == null) {
				...
				float firstXaxisPosition = (float) (getDataQuadrantWidth())
						* (1f - (firstPoint.getPosition() - mUnionCandleStickChart
								.getDrawOffset()) / (0f + drawStickNumber));
				float firstYaxisPosition = (float) ((1f - (firstPoint
						.getAxisYvalue() - minValue) / (maxValue - minValue)) * (super
				...
				if (secondPoint != null) {
					float secondXaxisPosition = (float) (getDataQuadrantWidth())
							* (1f - (secondPoint.getPosition() - mUnionCandleStickChart
									.getDrawOffset()) / (0f + drawStickNumber));
					float secondYaxisPosition = (float) ((1f - (secondPoint
							.getAxisYvalue() - minValue)
							/ (maxValue - minValue)) * (super.getHeight() * 2 / 3 - axisXTitleQuadrantHeight));
					if (Math.abs(event.getX() - secondXaxisPosition) <= 25
							&& Math.abs(event.getY() - secondYaxisPosition) <= 25) {
						isNearSeconde = true;
					} else {
						...
						GoldPointValue second = mGoldLineEntity
								.getSecondValue();
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (touchMode != ZOOM && touchMode != NONE) {
				if (isNearFirst) {
					...
					if (mLastMotionX < event.getX()
							&& mLastMotionY < event.getY()) { // 手指方向 向右 往左滑动
