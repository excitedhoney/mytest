...
public class FenshiChart extends IntervalGridView {
	...
	public void calcRange() {
		...
		for (int i = 0; i < lineData.size(); i++) {
			if (i == 0) {
				maxValue = lineData.get(i).getCurrentPrice();
				minValue = lineData.get(i).getCurrentPrice();
			} else {
				if (maxValue < lineData.get(i).getCurrentPrice()) {
					maxValue = lineData.get(i).getCurrentPrice();
				}
				if (minValue > lineData.get(i).getCurrentPrice()) {
					minValue = lineData.get(i).getCurrentPrice();
				}
			}
		}
