...
public class IntervalGridView extends BaseChart implements ITouchEventNotify,
		...
		if (axisYPosition == AXIS_Y_POSITION_LEFT) {
			...
			for (int i = 0; i < latitudeTitles.size(); i++) {
				if(i == 0) {
					...
					canvas.drawLine(startFrom, getDataQuadrantStartY(), startFrom
							+ length, getDataQuadrantStartY(), mPaintLine);
				}else{
					canvas.drawLine(startFrom, offset - (i - 1) * postOffset - postOffset / 4f, startFrom
							+ length, offset - (i - 1) * postOffset - postOffset / 4f, mPaintLine);
				}
			}
		}
	}
	...
	protected void drawLatitudeTitle(Canvas canvas) {
		...
		if (axisYPosition == AXIS_Y_POSITION_LEFT) {
			...
			for (int i = 0; i < latitudeTitles.size(); i++) {
				if (0 == i) {
					canvas.drawText(latitudeTitles.get(i), startFrom - mPaintFont.measureText(latitudeTitles.get(i)) ,
							super.getHeight() - this.axisXTitleQuadrantHeight
									- borderWidth - 2f, mPaintFont);
				} else if (i == latitudeTitles.size() - 1) {
					canvas.drawText(latitudeTitles.get(i), startFrom - mPaintFont.measureText(latitudeTitles.get(i)), offset
...
//							- i * postOffset + latitudeFontSize, mPaintFont);
					...
					canvas.drawText(latitudeTitles.get(i), startFrom, offset
							- (i - 1) * postOffset - postOffset / 4 + latitudeFontSize / 2f,
							mPaintFont);
				}
			}
		}
