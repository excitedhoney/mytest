...
public class EncryptUtil {
	...
	public static String encryptByMd5(String str){
		return StringUtil.getMD5String(str);
	}
	 ...
	 * Description:3des加密
	 * 属于加密之后的base64编码
	 * 同时使用默认的key=123456789012345678901234
	 * </p>
	 * 
	 * @param value
	 * @return
	 */
	public static String encryptBy3des(String value){
		return encryptBy3des("123456789012345678901234", value);
	}
	/***
	 * <p>
	 * Description:3des解密
	 ...
	 * @return
	 */
	public static String decryptBy3des(String value){
		return decryptBy3des("123456789012345678901234", value);
	}
	
	/***
	 * <p>
	 * Description:针对string进行3des加密
	 * 将string加密之后用base64编码
	 * </p>
	 * 
	 * @param key
	 * @param value
	 * @return
	 */
	public static String encryptBy3des(String key,String value){
		
		String encodeStr = null;
		try {
			byte[] encodeByte = encryptBy3des(key.getBytes(), value.getBytes());
//			BASE64Encoder encoder = new BASE64Encoder();
			encodeStr = new String(Base64.encodeBase64(encodeByte));
		} catch (Exception e) {
	 ...
	 * @return
	 */
	public static String decryptBy3des(String key,String value){
		String decodeStr = null;
		try {
//			BASE64Decoder decoder = new BASE64Decoder();
			byte[] decriptByte = decryptBy3des(key.getBytes(),Base64.decodeBase64(value.getBytes()));
			decodeStr = new String(decriptByte);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	private static String Alg_DESEDE = "DESede";
	/**
	 * <p>
	 * Description:针对byte进行3des解密
	 * 此过程不会将数据转成base64
	 * </p>
	 * 
	 * @param key
	...
	public static byte[] decryptBy3des(byte[] key, byte[] data)
			throws Exception {
		SecretKey secKey = new SecretKeySpec(key, Alg_DESEDE);
		Cipher c1 = Cipher.getInstance(Alg_DESEDE);
		c1.init(Cipher.DECRYPT_MODE, secKey);
		return c1.doFinal(data);
	}
	
	/***
	 * <p>
	 * Description:针对byte进行3des加密
	 * </p>
	 * 
	 * @param key
	 * @param data
	...
	public static byte[] encryptBy3des(byte[] key, byte[] data)
		...
		SecretKey secKey = new SecretKeySpec(key, Alg_DESEDE);
		Cipher c1 = Cipher.getInstance(Alg_DESEDE);
		c1.init(Cipher.ENCRYPT_MODE, secKey);
		return c1.doFinal(data);
	}
