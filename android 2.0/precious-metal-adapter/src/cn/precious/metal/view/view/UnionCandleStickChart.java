...
public class UnionCandleStickChart extends UnionGridChart {
	...
	protected boolean isSetTopMaxMinValue = false ;	

	private float stickWidth = 9f;
	...
	public void drawToolsLines(Canvas canvas) {
		if (lineTools == null || lineTools.isEmpty()) {
			return;
		}
	...
	private void calcRange() {
		...
		if (drawOffset <= 4) {
			for (int i = 5; i < 5 + drawStickNumber; i++) {
				if (i >= stickData.size())
					break;
				if (!isSetTopMaxMinValue && i== 5) {
			...
			for (int i = drawOffset; i < drawOffset + drawStickNumber; i++) {
				...
				if (!isSetTopMaxMinValue  && i== drawOffset) {
					maxValue = stickData.get(i).getHigh();
					minValue = stickData.get(i).getLow();
				} else {
					if (maxValue < stickData.get(i).getHigh()) {
	...
	protected void initAxisX() {
		...
		if (null != stickData && stickData.size() > 0) {
			...
			for (int i = 0; i <= this.getLongitudeNum(); i++) {
				...
				titleX.add(String.valueOf(stickData.get(index).getDate() != null ?stickData.get(index).getDate().substring(5) : ""));
			}
		}
	...
	protected void drawCurrentPriceLine(Canvas canvas) {
		if (null == stickData) {
			return;
		}
		if (stickData.size() <= 4) {
		...
		if (drawOffset <= 4) {
			entity = stickData.get(5);
		} else {
			entity = stickData.get(drawOffset);
		}
		...
		if (entity != null) {
			...
			float closeY = (float) ((1f - (entity.getZuoShou() - getMinValue())
					/ (getMaxValue() - getMinValue())) * realHeight);

			path.moveTo(getDataQuadrantStartX(), closeY);
			path.lineTo(getDataQuadrantStartX() + length, closeY);
			canvas.drawPath(path, mPaintLine);

			// canvas.drawLine(getDataQuadrantStartX(), closeY,
			// getDataQuadrantStartX() + length, closeY,
			// mPaintLine);

			if (closeY < getAxisXTitleQuadrantHeight() / 2f + borderWidth) {
				ptStart = new PointF(getDataQuadrantEndX(),
						getDataQuadrantStartX());
				ptEnd = new PointF(getDataQuadrantEndX()
						+ getAxisYTitleQuadrantWidth(),
						getAxisXTitleQuadrantHeight() + getDataQuadrantStartX());
			} else if (closeY > getDataQuadrantEndY()
					- getAxisXTitleQuadrantHeight() / 2f - borderWidth) {
				ptStart = new PointF(getDataQuadrantEndX(),
						getDataQuadrantEndY() - getAxisXTitleQuadrantHeight());
				ptEnd = new PointF(getDataQuadrantEndX()
						+ getAxisYTitleQuadrantWidth(), getDataQuadrantEndY());
			} else {
				ptStart = new PointF(getDataQuadrantEndX(), closeY
						- getAxisXTitleQuadrantHeight() / 2f);
				ptEnd = new PointF(getDataQuadrantEndX()
						+ getAxisYTitleQuadrantWidth(), closeY
						+ getAxisXTitleQuadrantHeight() / 2f);
			}
			drawCurrentPriceAlphaTextBox(ptStart, ptEnd,
					"" + entity.getZuoShou(), 18, canvas);
		}

	}

	private void drawCurrentPriceAlphaTextBox(PointF ptStart, PointF ptEnd,
			String content, int fontSize, Canvas canvas) {

		Paint mPaintBox = new Paint();
		mPaintBox.setColor(Color.WHITE);
		mPaintBox.setAlpha(30);
		mPaintBox.setStyle(Style.FILL);

		Paint mPaintBoxLine = new Paint();
		mPaintBoxLine.setColor(Color.GRAY);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (touchMode != ZOOM && touchMode != NONE  && isTop) {
	...
	protected boolean moveLeft(float offent) {
		...
		if (stickData.size() * getStickWidth() < (getDataQuadrantEndX() - getDataQuadrantStartX()))
			return false;
	 ...
	 * </p>
	 */
	public static final int DEFAULT_NEGATIVE_STICK_BORDER_COLOR = Color
			.parseColor("#8EE5EE");
