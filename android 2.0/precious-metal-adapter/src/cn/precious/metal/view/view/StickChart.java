...
import cn.precious.metal.view.entity.IChartData;
import cn.precious.metal.view.entity.IMeasurable;
import cn.precious.metal.view.entity.IStickEntity;
import cn.precious.metal.view.entity.StickEntity;
...
//				- stickSpacing;
		...
		if (axisYPosition == AXIS_Y_POSITION_LEFT) {
			...
			float stickX = getDataQuadrantPaddingEndX();
			for (int i = stickData.size() - 1 - drawOffset; i >=stickData.size() - 1 - drawOffset -drawStickNumber; i--) {
				if(i<= 0){
					i = 0 ;
				}
				IMeasurable stick = stickData.get(i);
			...
			if (touchMode != ZOOM && touchMode != NONE) {
				if (Math.abs(mLastMotionX - (int) event.getX()) > MIN_LENGTH) {
					...
					if (mLastMotionX < (int) event.getX()) { // 手指方向 向右 往左滑动
						moveLeft(xMoveOffset);
					} else {
						moveRight(xMoveOffset);
					}
					xMoveOffset = 0f ;
	...
	protected void moveLeft(float offent) {
		if (stickData == null || stickData.size() <= 0)
			return;
		int drawLineNumber = (int) (getDataQuadrantWidth() / stickWidth);
		int slideNumber = (int) (offent / stickWidth); // 滑动时偏移多少个数据
		...
		if (axisYPosition == AXIS_Y_POSITION_LEFT) {
			...
			if (drawOffset >= (size - drawLineNumber)) {
				drawOffset = size - drawLineNumber;
			}
		}

	}

	protected void moveRight(float offent) {
