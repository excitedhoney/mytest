...
public class GridChart extends BaseChart implements ITouchEventNotify,
	 ...
	 * </p>
	 */
	public static final int DEFAULT_LATITUDE_FONT_SIZE = 28;
	...
	protected float getDataQuadrantHeight() {
		return super.getHeight() - axisXTitleQuadrantHeight - borderWidth;
	}
	...
	protected void drawLongitudeLine(Canvas canvas) {
		...
		if (counts > 1) {
			float postOffset = (this.getDataQuadrantWidth() - borderWidth)
					/ (counts - 1);

			float offset;
			if (axisYPosition == AXIS_Y_POSITION_LEFT) {
				offset = getDataQuadrantStartX();
			} else {
				offset = borderWidth + dataQuadrantPaddingLeft;
			}
			for (int i = 0; i < counts; i++) {
				canvas.drawLine(offset + i * postOffset,
						getDataQuadrantStartY(), offset + i * postOffset,
						length, mPaintLine);
			}
		}
	}
		...
		for (int i = 0; i < longitudeTitles.size(); i++) {
			if (0 == i) {
				...
				canvas.drawText(longitudeTitles.get(i), offset + i * postOffset
						...
						* longitudeFontSize / 4f, super.getHeight() - 2f,
						mPaintFont);
			}
	...
	protected void drawLatitudeLine(Canvas canvas) {
		...
		if (null == latitudeTitles || latitudeTitles.isEmpty()) {
			return;
		}
		if (!displayLatitude) {
			return;
		...
		float offset = super.getHeight() - axisXTitleQuadrantHeight
				- dataQuadrantPaddingBottom;
		float startOffset = getDataQuadrantStartY();

		float postOffset = (this.getDataQuadrantEndY() - getDataQuadrantStartY())
				/ (latitudeTitles.size() - 1);

		if (axisYPosition == AXIS_Y_POSITION_LEFT) {
			float startFrom = getDataQuadrantPaddingStartX();
			for (int i = 0; i < latitudeTitles.size(); i++) {
				canvas.drawLine(startFrom, startOffset + i * postOffset,
						startFrom + length, startOffset + i * postOffset,
						mPaintLine);
