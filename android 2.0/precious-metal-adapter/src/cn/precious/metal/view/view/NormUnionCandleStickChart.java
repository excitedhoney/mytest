...
public class NormUnionCandleStickChart extends UnionCandleStickChart {
	...
	public void calcTopRange() {
		if (null == linesData || linesData.isEmpty())
			return;

		isSetTopMaxMinValue = false;

		int drawStickNumber = (int) (getDataQuadrantWidth() / getStickWidth());

		if (NORM_BOLL.equalsIgnoreCase(topNormType)) {
			for (int i = 0; i < linesData.size(); i++) {
				List<KLineEntity> list = linesData.get(i).getLineData();
				if (list != null && !list.isEmpty()) {
					if (getDrawOffset() <= 6) {
						for (int j = list.size() - 6; j >= list.size()
								- drawStickNumber - 6; j--) {
							if (j < 0)
								break;

							if (i == 0 && j == list.size() - 6) {
								maxValue = list.get(j).getNormValue();
								minValue = list.get(j).getNormValue();
							} else {
								if (maxValue < list.get(j).getNormValue()) {
									maxValue = list.get(j).getNormValue();
								}

								if (minValue > list.get(j).getNormValue()) {
									minValue = list.get(j).getNormValue();
								}
							}
						}
					} else {
						for (int j = list.size() - getDrawOffset(); j >= list
								.size() - drawStickNumber - getDrawOffset(); j--) {
							if (j < 0)
								break;

							if (i == 0 && j == list.size() - getDrawOffset()) {
								maxValue = list.get(j).getNormValue();
								minValue = list.get(j).getNormValue();
							} else {
								if (maxValue < list.get(j).getNormValue()) {
	...
	public void calcButtonRange() {
		...
		for (int j = 0; j < buttomNormLineDatas.size(); j++) {
			List<KLineEntity> lineData = buttomNormLineDatas.get(j)
					.getLineData();
		...
		if (NORM_MACD.equalsIgnoreCase(buttomNormType)) {
			if (null == buttomStickData || buttomStickData.isEmpty()) {
				return;
			}

			for (int i = 0; i < buttomStickData.size(); i++) {
				if (buttomMaxValue < buttomStickData.get(i).getNormValue()) {
					buttomMaxValue = buttomStickData.get(i).getNormValue();
				}

				if (buttomMinValue > buttomStickData.get(i).getNormValue()) {
					buttomMinValue = buttomStickData.get(i).getNormValue();
				}
			}
		}
	...
	protected void drawSMaLines(Canvas canvas) {
		...
		for (int i = 0; i < linesData.size(); i++) {
			...
			if (getDrawOffset() <= 4) {
				for (int j = lineData.size() - 1 - getDrawOffset(); j > lineData
						.size() - 1 - drawLineNum - getDrawOffset(); j--) {
					if (j < 0) {
						break;
					}
					...
					if(j > lineData.size() - 6 ) {
						...
						// calculate Y
						valueY = (float) ((1f - (value - minValue)
	...
	protected void drawBoolLines(Canvas canvas) {
		...
		for (int i = 0; i < linesData.size(); i++) {
			...
			if (getDrawOffset() <= 4) {
				...
				for (int j = lineData.size() - getDrawOffset() - 1; j > lineData
						.size() - getDrawOffset() - 1 - drawLineNum; j--) {
					if (j < 0) {
						break;
					}
					...
					if (j > lineData.size() - 6) {
						...
						// float value = (float) lineData.get(lineData.size() -
						// 6).getNormValue();
						// // calculate Y
						// valueY = (float) ((1f - (value - minValue)
						// / (maxValue - minValue)) * (super.getHeight() * 2 / 3
						// - axisXTitleQuadrantHeight));
						// // if is not last point connect to previous point
						// if (valueY >= super.getHeight() * 2 / 3
						// - axisXTitleQuadrantHeight) {
						// valueY = super.getHeight() * 2 / 3
						// - axisXTitleQuadrantHeight;
						// }
						//
						// if (j != lineData.size() - 1 - getDrawOffset()) {
				...
				for (int j = lineData.size() - 1 - getDrawOffset(); j > lineData
						.size() - 1 - getDrawOffset() - drawLineNum; j--) {
					if (j < 0) {
						break;
					}
	...
	protected void drawMacdStick(Canvas canvas) {
		...
		mPositivePaintFill.setStrokeWidth(2);
		mPositivePaintFill.setStyle(Style.STROKE);

		Paint mNegativePaintFill = new Paint();
		mNegativePaintFill.setStyle(Style.STROKE);
		mNegativePaintFill.setStrokeWidth(2);
		mNegativePaintFill.setColor(Color.parseColor("#8EE5EE"));

		int drawStickNumber = (int) (getDataQuadrantWidth() / (getStickWidth() + stickSpacing));

		float stickX = getDataQuadrantEndX() - getStickWidth();

		for (int i = buttomStickData.size() - 1 - getDrawOffset(); i > buttomStickData
				.size() - 1 - getDrawOffset() - drawStickNumber; i--) {
			if (i < 0) {
				break;
			}
			...
			// draw stick
			if (stick.getHigh() == 0) {
				// canvas.drawRect(stickX, highY, stickX + getStickWidth(),
				// lowY, mNegativePaintFill);
				canvas.drawLine(stickX + getStickWidth() / 2f, highY, stickX
						+ getStickWidth() / 2f, lowY, mNegativePaintFill);
			} else if (stick.getLow() == 0) {
				// canvas.drawRect(stickX, highY, stickX + getStickWidth(),
				// lowY,mPositivePaintFill);
				canvas.drawLine(stickX + getStickWidth() / 2f, highY, stickX
						+ getStickWidth() / 2f, lowY, mPositivePaintFill);
			}
