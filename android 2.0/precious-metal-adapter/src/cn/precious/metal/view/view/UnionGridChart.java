...
public class UnionGridChart extends BaseChart implements ITouchEventNotify,
	 ...
	 * </p>
	 */
	public static final int DEFAULT_LAITUDE_COLOR = Color.WHITE;

	/**
	 * 
	 * 
	 */
	public static final float DEFAULT_AXIS_Y_TITLE_QUADRANT_WIDTH = 18f;
	 ...
	 * </p>
	 */
	public static final float DEFAULT_AXIS_X_TITLE_QUADRANT_HEIGHT = 18f;
	 ...
	 * </p>
	 */
	public static final int DEFAULT_LATITUDE_FONT_COLOR = Color.WHITE;
	...
	protected void drawLatitudeLine(Canvas canvas) {
		...
		if (null == latitudeTopTitles || latitudeTopTitles.isEmpty()) {
			...
			float postOffset = (super.getHeight() * 2 / 3 - axisXTitleQuadrantHeight)
					/ (getLatitudeTopTitles().size());
			float buttomPostOffset = (super.getHeight() * 1 / 3)
					/ (getLatitudeButtomTitles().size());
			...
			float startFrom = getDataQuadrantStartX();
			if (axisYPosition == AXIS_Y_POSITION_RIGHT) {
				for (int i = 0; i <= getLatitudeTopTitles().size(); i++) {
					if (i == 0) {
						canvas.drawLine(startFrom, offset, startFrom + length,
								offset, mPaintLine);
					} else {
				...
				for (int i = 0; i <= getLatitudeButtomTitles().size(); i++) {
					if (i == latitudeButtomTitles.size() - 1) {
						canvas.drawLine(startFrom, super.getHeight() - borderWidth,
								startFrom + length,
								super.getHeight() - borderWidth, mPaintLine);
