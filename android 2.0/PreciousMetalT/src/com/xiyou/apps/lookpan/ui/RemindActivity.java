...
public class RemindActivity extends BaseActivity implements OnClickListener {
	...
	private TextView optionaName, optioanlSale;
	...
	public void chooseOptional(Intent intent) {
		...
		BuyPrice = intent.getDoubleExtra("BuyPrice",0) ;
		...
		if (intent.getStringExtra("name") != null
			...
			// optionalBuy.setText("买入价：" + intent.getDoubleExtra("BuyPrice",
			// 0));
			optioanlSale
	...
	public void onClick(View v) {
		...
		case R.id.btn_add_notice:
			...
			notice.setName(name);
			notice.setSymbol(symbol);
			notice.setBuyPrice(BuyPrice);
			notice.setSalePrice(SalePrice);
			
			String cprice = optionalCurrentPrice.getText().toString() ;
			if(cprice == null || "".equals(cprice)) {
				notice.setSetPrice(CurrentPrice);
			}else{
