...
import cn.precious.metal.adapter.ChooseOptionalAdapter;
...
public class ChooseOptionalActivity extends BaseActivity implements OnClickListener{
	...
	public void initView() {
		...
		sjsList.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				if(op != null) {
					...
					it.putExtra("SalePrice", Double.parseDouble(op.getSellone())) ;
					it.putExtra("CurrentPrice", Double.parseDouble(op.getNewest())) ;
		...
		tjsListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
	...
	public void onClick(View v) {
		...
		case R.id.ll_sjs:
			...
			if (AppSetting.getInstance(this).isHaveInitSJSOptionalData()) {
				options1 = new OptionalDao(this).queryOptionalsByType(ParamConfig.SHANG_JIAO_SUO);
				optionalAdapter1.setItems(options1);
				Utils.setListViewHeightBasedOnChildren(ChooseOptionalActivity.this, sjsList, false);
			} else {
		...
		case R.id.ll_tjs:
			if (AppSetting.getInstance(this).isHaveInitTJSOptionalData()) {
				options2 = new OptionalDao(this).queryOptionalsByType(
						ParamConfig.JIN_GUI_SUO);
				optionalAdapter2.setItems(options2);
				Utils.setListViewHeightBasedOnChildren(ChooseOptionalActivity.this, tjsListView, false);
			} else {
