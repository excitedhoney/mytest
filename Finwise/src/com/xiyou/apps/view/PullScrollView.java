 ...
 * 自定义ScrollView
 * 
 * @author MarkMjw [url=home.php?mod=space&uid=354393]@DATE[/url] 2013-09-13
 */
public class PullScrollView extends ScrollView {
	...
	private void doTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (mHeadImage != null) {
				mCurrentTop = mInitTop = mHeadImage.getHeight();
	...
	private void doActionMove(float deltaY) {
		...
		if (isMoving) {
			...
			float bgMoveH = deltaY * 0.4f * SCROLL_RATIO;
			mCurrentTop = (int) (mInitTop + bgMoveH);
			mCurrentBottom = (int) (mInitBottom + bgMoveH);

			if (mHeadImage != null) {
				mHeadImage.layout(mHeadImage.getLeft(), mCurrentTop,
						mHeadImage.getRight(), mCurrentBottom);
			}
	...
	private void rollBackAnimation() {
		...
		if (mHeadImage != null) {
			...
			mHeadImage.layout(mHeadImage.getLeft(), mInitTop,
					mHeadImage.getRight(), mInitBottom);
